...
AuthService(this.firebaseAuth);
  ...
  Future<void> loginUser({required String email, required String password}) async{
     try{
       final g = await firebaseAuth.signInWithEmailAndPassword(email: email, password: password);
       await OneSignal.login(g.user!.uid);
  ...
  Future<void> registerUser({required String email, required String password, required String username}) async{
    ...
    await OneSignal.login(user.user!.uid);
    } on FirebaseAuthException catch(err){
      throw '${err.message}';
    }
  }
